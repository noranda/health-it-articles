...
namespace :tags do
  ...
  task :generate do
    ...
    Dir.glob("#{base_dir}/articles/tag/*.md").each do |file|
      puts "Removing file: #{file}"
      File.unlink(file)
    end

    tags_data.each do |tag_data|
      puts "Writing articles/tag/#{tag_data['slug']}.md"

      File.open("#{base_dir}/articles/tag/#{tag_data['slug']}.md", 'w') do |f|
        f.write(YAML.dump({
          'layout' => 'article_by_tag',
          'tag' => tag_data['name'],
          'permalink' => "articles/tag/#{tag_data['slug']}/"
        }))
        f.write('---')
      end
    end
